...
<!-- isinya -->
<h1 class="h3 mb-0">
  Data Perubahan Akta (Pengadilan Negeri)
...
<table class="table table-striped table-sm table-bordered dt-responsive nowrap" id="table" width="100%">
  ...
  <tbody>
    <?php
    $no = 1;
    $data_akta = mysqli_query($conn, "SELECT * FROM akta_pn 
                                 LEFT JOIN jenis_keterangan ON akta_pn.id_keterangan = jenis_keterangan.id_keterangan
    ...
    if (isset($_POST['TambahAkta'])) {
      ...
      $cek_akta = mysqli_num_rows(mysqli_query($conn, "SELECT * FROM akta_pn WHERE nomor_akta='$nomor_akta'"));
      if ($cek_akta > 0) {
        echo '<script>alert("Nomor Akta sudah ada");history.go(-1);</script>';
      } else {
        $query = "INSERT INTO akta_pn (nomor_akta, nama_pemilik, jenis_akta, tanggal_terbit, id_keterangan)
    ...
    if (isset($_POST['SimpanEditAkta'])) {
      ...
      $query = "UPDATE akta_pn SET nomor_akta='$nomor_akta', nama_pemilik='$nama_pemilik', jenis_akta='$jenis_akta', 
    ...
    if (!empty($_GET['hapus'])) {
      ...
      $query = mysqli_query($conn, "DELETE FROM akta_pn WHERE nomor_akta='$nomor_akta'");
      if ($query) {
        echo '<script>history.go(-1);</script>';
      } else {
